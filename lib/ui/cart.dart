...
class Cart extends StatelessWidget {
  ...
  Cart({Key key, this.total}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return MaterialApp(
        debugShowCheckedModeBanner: false, home: CartPage(total: total));
  }
}
...
class CartPage extends StatefulWidget {
  ...
  CartPage({Key key, this.total}) : super(key: key);
}
...
class _CartPageState extends State<CartPage> {
  ...
  queryValues(idItem, priceAkhir) async {
    ...
    Firestore.instance
        ...
        .collection('item')
        .document(idItem)
        .collection('supplier_sender')
        .snapshots()
        .listen((snapshot) {
      int tempTotal = snapshot.documents
          .fold(0, (tot, doc) => tot + doc.data['stok_tambah']);
      ...
      Firestore.instance
          ...
          .document(idItem)
          .updateData(
              {'item_qty': totalHarga, 'item_supplierprice': priceAkhir});
      print(totalHarga);
      return totalHarga;
      ...
      child: Column(
        children: <Widget>[
          Stack(children: <Widget>[
            ...
            Container(
                height: 100.0, width: double.infinity, color: Colors.orange),
            Padding(
                  ...
                  if (snapshot.data != null) {
                    return Column(
                        children: snapshot.data.documents
                            .map((doc) => itemCard(record.name, 'gray', '248',
                                'assets/img/login_logo.png', true, 1, doc))
                            .toList());
                  } else {
          ...
          hargaPromo
              ? Text(
                  'Total: \Rp  ${hargaTotal.toString()} (${discountPrice.toString()})')
  ...
  applyPromo(promoCode) async {
    ...
    var userQuery = await db
        ...
        .document(idUser)
        .collection('promo')
        .where('promo_code', isEqualTo: promoCodeStr)
        .getDocuments();
    ...
    if (available == null) {
      ...
      Firestore.instance
          ...
          .listen((snapshot) {
        int tempTotal =
            snapshot.documents.fold(0, (tot, doc) => tot + doc.data['qty']);
        totalItem = tempTotal;
        // debugPrint(totalHarga.toString());
    ...
    if (method == "Cash") {
      if (kembalian >= 0 || kembalian == 0) {
        ...
        Firestore.instance
            ...
            .updateData({'id_header': ref.documentID});
  ...
  lifo(idHeader, kembalian, hargaBayar) async {
    CollectionReference cartRef = Firestore.instance
        .collection('owner')
        .document(idUser)
        .collection('cart');
    ...
    for (DocumentSnapshot ds in eventsQuery.documents) {
      ...
      Firestore.instance
          ...
          .document(idUser)
          .collection('item')
          .document(ds.data['id_item'].toString())
          .collection('supplier_sender')
            ...
            if (n <= 0) {
              ...
              try {
                ...
                Firestore.instance
                    ...
                    .document(ds.data['id_item'].toString())
                    .collection('supplier_sender')
                    .document(snapshot.documents[indexArr].documentID)
                    .updateData({'stok_tambah': n});
              } catch (e) {}
              ...
              try {
                ...
                int priceAkhir =
                    snapshot.documents[indexArr].data['harga_beli'];
  ...
  Widget itemCard(
      itemName, color, price, imgPath, available, i, DocumentSnapshot doc) {
